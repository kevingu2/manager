...
class BrowseController < ApplicationController
  def index
    ...
    @within30days = Oppty.where("proposalDueDate >= ? and proposalDueDate <= ?", Date.today.to_s, thirty.to_s).page(params[:page]).per_page(15).order(params[:sort])
    @within60days = Oppty.where("proposalDueDate >= ? and proposalDueDate <= ?", Date.today.to_s, sixty.to_s).page(params[:page]).per_page(15).order(params[:sort])
    @within90days = Oppty.where("proposalDueDate >= ? and proposalDueDate <= ?", Date.today.to_s, ninety.to_s).page(params[:page]).per_page(15).order(params[:sort])
